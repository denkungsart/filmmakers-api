...
picture_version | null | Can be set to `original`, `large`, `thumb` or `thumb_portrait` to change the included picture version. The picture will be included in a field named `picture_url`. _(Only applies if `include_picture` is true)_
